...
do
  ...
  vim.keymap.set('n', ']b', '<cmd>normal! ]]<CR>', { desc = 'Next section' })
  vim.api.nvim_create_autocmd('FileType', {
    desc = 'Keep buffer navigation mappings after filetype plugins load',
    callback = function(args)
      vim.keymap.set('n', '[[', '<cmd>bprevious<CR>', { buffer = args.buf, desc = 'Previous buffer' })
      vim.keymap.set('n', ']]', '<cmd>bnext<CR>', { buffer = args.buf, desc = 'Next buffer' })
    end,
  })
...
do
  ...
  -- Rainbow delimiters colour matching nested brackets. It uses Treesitter, so it
  -- works for every filetype with a supported parser.
  ...
  -- Install parsers for the languages you use. Parsers for other filetypes are
  -- still installed automatically when their buffers are first opened.
  local parsers = {
    'bash',
    'c',
    'cpp',
